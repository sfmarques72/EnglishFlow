...
function parseGeminiJson(text: string) {
  ...
  try {
    ...
    return {
      reply: text,
      correction:
        "💡 Dica de Inglês: Continue praticando! A conversa está indo muito bem.",
    };
  }
}

export type CreateAppOptions = {
  /** When false (Vercel), static files are served by the CDN, not Express. */
  ...
  app.post("/api/chat", async (req, res) => {
    try {
      ...
      if (!process.env.GEMINI_API_KEY || !ai) {
        ...
        let reply =
          "That sounds great! I would love to learn more about you. What do you usually do in your free time?";
        let correction =
          "💡 Dica de Inglês (Modo Simulação): Excelente esforço! Você usou palavras simples e corretas. (Insira sua chave GEMINI_API_KEY para respostas reais da IA).";
        ...
        if (
          ...
          reply =
            "Hello! It's so nice to meet you. My name is Sarah. Today we are talking about " +
            currentTopic +
            ". Are you ready to practice English with me?";
          correction =
            "💡 Dica de Inglês: Quando cumprimentar alguém, 'Nice to meet you' (Prazer em conhecer você) é uma ótima frase de início!";
        } else if (
          lastUserMessage.toLowerCase().includes("yes") ||
          lastUserMessage.toLowerCase().includes("ready")
        ) {
          reply =
            "Wonderful! Let's start. Tell me: " +
            (currentTopic.toLowerCase().includes("rotina")
              ? "What is your morning routine? For example: I wake up at 7 AM."
              : "What do you like to do on weekends?");
          correction =
            "💡 Dica de Inglês: Para falar sobre rotinas, usamos o Present Simple. Exemplo: 'I study English' (Eu estudo inglês).";
...
Ensure the response is valid JSON. Do not wrap in markdown code blocks like \`\`\`json. Return only the JSON.`;
      ...
      const formattedHistory = messages
        .slice(-5)
        .map((m: any) => `${m.role === "user" ? "Student" : "Sarah"}: ${m.content}`)
        .join("\n");
